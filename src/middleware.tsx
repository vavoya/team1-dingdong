 ...
 * 에러 모달을 렌더링하는 함수
 */
function renderErrorModal(render: (modal: ReactNode) => void, title: string, text: string) {
    render(
        <Modal
            title={[title]}
            text={[text]}
...
function renderErrorModal2(render: (modal: ReactNode) => void, title: string, text: string, button: {text: string, onClick: () => void}) {
    render(
        <Modal
            title={[title]}
            text={[text]}
 ...
 * Axios 에러를 처리하는 함수
 */
function handleAxiosError(error: any, currentUrl: string, render: (modal: ReactNode) => void) {
    ...
    if (status) {
        ...
        renderErrorModal(render, "서버 응답이 없음", "서버에서 응답을 받을 수 없습니다.");
    }
}
 ...
 */
function handleError(error: unknown, currentUrl: string, render: (modal: ReactNode) => void) {
    if (axios.isAxiosError(error)) {
        handleAxiosError(error, currentUrl, render);
    } else if (error instanceof TypeError) {
        renderErrorModal(render, "네트워크 문제 또는 API 오류", "네트워크 문제로 요청을 처리할 수 없습니다.");
    } else if (error instanceof Error) {
        renderErrorModal(render, "일반적인 오류 발생", "알 수 없는 오류가 발생했습니다.");
    } else {
        renderErrorModal(render, "알 수 없는 오류 발생", "예기치 않은 문제가 발생했습니다.");
    }
