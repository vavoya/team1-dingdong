...
export default function useToast(bottom: string = '-20px') {
    ...
    useEffect(() => {
        ...
        return () => {
            if (rootRef.current != null) {
                // 토스트 컨테이너 제거
                unmountToastContainer(rootRef.current.root, rootRef.current.toastContainer)
            }
        }
    }, [])
...
function reserveUnmountToast ({toastSet, toast, root, toastContainer, bottom}: ReserveUnmounToastProps) {
    setTimeout(() => {
        // 토스트 아래로 사라지게
        toast.bottom = '100%'
