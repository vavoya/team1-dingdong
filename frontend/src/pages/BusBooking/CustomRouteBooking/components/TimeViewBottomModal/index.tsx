...
import { CommuteType, OverlayBottomModalType } from "@/pages/BusBooking/types/commuteType";
...
import { convertIsoToDateObject, convertToFormattedTime } from "@/utils/calendar/timeViewBottomModalUtil";
...
export default function TimeViewBottomModal({
  ...
  const editIconHandler = (timeScheduleIndex: number) => {
    setSelectedDate(convertIsoToDateObject(selectedTimeSchduleArray[timeScheduleIndex]));
    ...
    >
      ...
      <S.SelectedScheduleWrapper>
        <S.SubHeader>
          ...
          <S.ArrivalTime>{commuteType === "등교" ? "학교 도착 시각" : "집으로 출발 시각"}</S.ArrivalTime>
        </S.SubHeader>
        <S.ScheduleBoxWrapper $maxHeight={modalType}>
          {convertToFormattedTime(selectedTimeSchduleArray).map(([dateInfo, time], timeScheduleIndex) => (
            <S.ScheduleBox key={dateInfo} onClick={() => editIconHandler(timeScheduleIndex)}>
      ...
      {modalType === "lastStep" && (
        <S.ButtonBox>
          <CancelButton text="이전" onClick={() => setIsEditablTimeViewModalOpen(false)} />
